...
typedef uint32_t  tBTA_RDS_FEATURE;
...
typedef uint8_t tBTA_RDS_STATUS;
...
typedef uint8_t  tBTA_FM_RDS_TP_TYPE;
...
typedef uint8_t   tBTA_RDS_PTY_VAL;
...
typedef struct
{
    ...
    uint8_t               *p_str;  /* PTY display string */
} tBTA_RDS_PTY;
...
typedef uint8_t tBTA_RDS_M_S;

/* transprant data channell structure */
typedef struct
{
    uint8_t       chnl_num;   /* channel number */
    uint8_t       len;        /* data length */
    uint8_t       tdc_seg[4]; /* TDC data, max to 4 bytes */
...
typedef struct
{
    uint8_t               data_5b;        /* the 5 bits data carried in 2nd block */
    uint16_t              data_16b_1;     /* 16 bits data carried in block 3 if group type A */
    uint16_t              data_16b_2;     /* 16 bits data carried in block 4 */
} tBTA_RDS_SEG_DATA;

/* ODA */
typedef struct
{
    uint16_t              aid;        /* ODA application ID */
    uint8_t               grp_type;   /* group type which is carrying this AID */
    union
    {
        tBTA_RDS_SEG_DATA       msg_body;    /* ODA message information */
        uint16_t                  sys_info;    /* ODA system information carried in group 3A block C */
    }                           data_info;
}tBTA_RDS_ODA_DATA;
...
typedef uint8_t   tBTA_RDS_SLC_TYPE;
...
typedef union
{
    ...
    uint16_t              tmc_id;         /* TMC ID */
    uint16_t              paging_id;      /* paging ID */
    uint16_t              ews_channel;    /* EWS channel ID */
    uint16_t              other;           /* broadcaster data */
}  tBTA_RDS_SLC ;
...
typedef uint8_t tBTA_RDS_RPTOR_SET;
...
typedef uint8_t tBTA_RDS_EON_CODE;
...
typedef struct
{
    ...
    uint8_t   eg;     /* Extended Generic indicator */
    uint8_t   ils;    /* Internaltional Linkage Set Indicator */
    uint8_t   lsn_ci;    /* Linkage Set Number : contry identifier */
    uint8_t   lsn_li;    /* Linkage Set Number: linkage identifier */
...
typedef uint8_t tBTA_RDS_AF_M_TYPE;
...
typedef union
{
    ...
    uint16_t              pi_code;       /* PI code */
    uint8_t               data8;         /* uint8_t data, used for DI callback */
    tBTA_FM_RDS_TP_TYPE ta_tp;          /* TP/TA status */
    uint8_t               *p_data;        /* data pointer to uint8_t, used for PS, PTYN */
    ...
    tBTA_RDS_RTP        rtplus;
    uint32_t              stats[4];       /* statistics data */
    tBTA_RDS_RT         rt;             /* used for RT */
} tBTA_FM_RDS;

typedef void (tBTA_RDS_CBACK)(tBTA_RDS_EVT event, tBTA_FM_RDS *p_data, uint8_t app_id);
...
extern tBTA_RDS_STATUS BTA_FmRDSRegister(tBTA_FM_RDS_B mode, tBTA_RDS_FEATURE rds_mask,
                                      tBTA_RDS_CBACK *p_cback, uint8_t app_id);
...
**
*******************************************************************************/
extern tBTA_RDS_STATUS BTA_FmRDSResetDecoder(uint8_t app_id);
/*******************************************************************************
**
...
**
*******************************************************************************/
extern tBTA_RDS_STATUS BTA_FmRDSDecode(uint8_t app_id, uint8_t *p_data, uint16_t length);
