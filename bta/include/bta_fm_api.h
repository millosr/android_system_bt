...
typedef uint8_t   tBTA_FM_STATUS;
...
typedef uint16_t   tBTA_FM_FUNC_MASK;
...
typedef uint8_t  tBTA_FM_REGION_CODE;
...
typedef uint8_t tBTA_FM_DEEMPHA_TIME;

#define BTA_FM_GET_FREQ(x)  ((uint16_t)((x * 10) - 64000))
...
typedef uint8_t tBTA_FM_SCH_RDS_TYPE;
...
typedef uint8_t  tBTA_FM_AUDIO_MODE;
...
typedef uint8_t  tBTA_FM_AUDIO_QUALITY;
...
typedef uint8_t tBTA_FM_AUDIO_PATH;
...
typedef uint8_t tBTA_FM_SCAN_METHOD;
...
typedef uint8_t tBTA_FM_SCAN_DIR;
...
typedef uint8_t tBTA_FM_SCAN_MODE;

#define  BTA_FM_STEP_100KHZ     0x00
#define  BTA_FM_STEP_50KHZ      0x10
typedef uint8_t   tBTA_FM_STEP_TYPE;
...
typedef struct
{
    uint8_t       start_snr;  /* Above this SNR, the chip will be in stereo mode
                               if the RSSI is high enough, valid range 0 -63 default 43 */
    uint8_t       stop_snr;   /* Below this SNR, the chip will be in MONO mode. valid range 0 -63, default 20*/
    int8_t        start_rssi; /* RSSI to start blending, assuming the FMRX_START_SNR
                               is exceeded. valid range -128 ~ 127, default - 56 */
    int8_t        stop_rssi;   /* RSSI to stop blending, assuming FMRX_STOP_SNR is
                               not yet exceeded. valid range -128 ~ 127, default - 85 */
}tBTA_FM_BLEND_CFG;

typedef struct
{
    uint8_t       start_mute; /* SNR to start muting. Below this SNR the chip will
                               partially mute until the ultimate mute attenuation
                               is reached. valid range 0 - 63, default 19 */
    int8_t       stop_atten; /* Ultimate mute attenuation level. valid range -128 ~ 127, default 26 */
    uint8_t      mute_rate;  /* Rate at which muting occurs valid range 0 - 63, default 8 */
    int8_t       snr40;     /* constant to add to SNR reading to get the actual SNR,
...
typedef union
{
    int8_t   snr;            /* valid SNR value range from 0 ~ BTA_FM_SNR_MAX */
    int8_t   cos;            /* valid COS slope value range from BTA_FM_COS_MIN ~ BTA_FM_COS_MAX */
}tBTA_FM_CVALUE;
...
typedef uint8_t tBTA_FM_AF_TYPE;

/* AF structure */
typedef struct
{
    uint16_t              num_af;     /* number of AF in list */
    tBTA_FM_AF_TYPE     af_type[BTA_FM_AF_MAX_NUM];
    uint16_t              af_list[BTA_FM_AF_MAX_NUM];
}tBTA_FM_AF_LIST;

typedef struct
{
    uint16_t              pi_code;          /* currently tuned frequency PI code */
    tBTA_FM_AF_LIST     af_list;          /* AF frequency list */
    uint8_t               af_thresh;        /* AF jump RSSI threshold*/
} tBTA_FM_AF_PARAM;
...
typedef uint8_t tBTA_FM_NFE_LEVL;
...
typedef struct
{
    ...
    uint8_t               rssi;
    int8_t                snr; /* current channel SNR value */
    uint16_t              freq;           /* tuned frequency */
}tBTA_FM_CHNL_DATA;
...
typedef struct
{
    uint8_t               rssi;
    uint16_t              freq;
    int8_t                cos; /* carrier offset */
    int8_t                snr; /* only valid when preset sscan mode 3 is used, otherwise set to 0 */
}tBTA_FM_SCAN_DAT;
...
typedef struct
{
    uint16_t                  low_bound;      /* lowest frequency boundary */
    uint16_t                  high_bound;     /* highest frequency boundary */
    tBTA_FM_DEEMPHA_TIME    deemphasis;     /* FM de-emphasis time constant */
    uint8_t                   scan_step;      /* scanning step */
} tBTA_FM_CFG_ENTY;
...
typedef struct
{
    ...
    int32_t                   aud_timer_value;/* audio quality live updating timer
                                               value, must be a non-0 positive value */
    int32_t                   rds_timer_value;/* RDS data update timer */
    int32_t                   enable_delay_val;/* FM enable delay time value, platform dependant */
    uint8_t                   rds_read_size;  /* how many RDS tuples per read */
    uint8_t                   max_station;    /* maximum number of FM stations for a full band scan
...
extern void BTA_FmEnable(tBTA_FM_FUNC_MASK func_mask,
                                 tBTA_FM_CBACK *p_cback, uint8_t app_id);
...
**
*******************************************************************************/
extern void BTA_FmSearchFreq(tBTA_FM_SCAN_MODE scn_mode, uint8_t rssi_thresh,
...
**
*******************************************************************************/
extern void BTA_FmTuneFreq (uint16_t freq);
...
**
*******************************************************************************/
extern void BTA_FmSetRDSMode (bool rds_on, bool af_on, tBTA_FM_AF_PARAM *p_af_struct);
...
**
*******************************************************************************/
extern void BTA_FmReadAudioQuality(bool turn_on);
...
**
*******************************************************************************/
extern void BTA_FmMute(bool mute);
...
**
*******************************************************************************/
extern void BTA_FmComboSearch(uint16_t start_freq, uint16_t end_freq, uint8_t rssi,
                      tBTA_FM_SCAN_DIR direction, tBTA_FM_SCAN_METHOD scn_method,
                      bool multi_chnl, tBTA_FM_SCH_RDS_COND * p_rds_cond);
...
**
*******************************************************************************/
extern tBTA_FM_STATUS BTA_FmSetSignalNotifSetting(int32_t poll_interval);
...
**                  on start is 255.
**
** Parameters       volume: is an uint16_t type, value range from 0 to 255, where
**                          255 indicates the highest volume.
**
** Returns          void
**
*******************************************************************************/
extern void BTA_FmVolumeControl(uint16_t volume);
