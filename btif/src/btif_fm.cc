...
******************************************************************************/
    ...
    static int btif_fm_set_rds_mode(bool rdsOn, bool afOn);
    static void btif_fm_set_rds_mode_int(bool rds_on, bool af_on);
    ...
    **
    ** Returns          bool: TRUE: when AF list is updated
    **                           FALSE: No AF list updating needed.
    **
    *******************************************************************************/
    static bool btif_fm_af_merge_list(tBTA_FM_AF_LIST *p_af_data)
    {
        uint8_t i, j ;
        bool duplicate, add_up = FALSE;
    ...
    ** Returns          void
    *******************************************************************************/
    void btif_fm_rdsp_cback(tBTA_RDS_EVT event, tBTA_FM_RDS *p_data, uint8_t app_id)
    {
        uint8_t   i = 0;
        bool new_af = TRUE;
...
static void btif_fm_set_rds_mode_int(bool rds_on, bool af_on)
{
    ...
    if (af_on && btif_fm_data.af_cb.af_avail &&
        ((p_af_param = (tBTA_FM_AF_PARAM *)osi_malloc(sizeof(tBTA_FM_AF_PARAM))) != NULL))
    {
        p_af_param->af_thresh = (uint8_t) BTIF_FM_AF_THRESH;
...
{
    bool af_on = btif_fm_data.af_cb.af_on ;
...
static void btif_fm_upstreams_evt(uint16_t event, char* p_evt_data)
{
    ...
    switch (event)
    {
        ...
        case BTA_FM_RDS_MODE_EVT:
            APPL_TRACE_DEBUG("%s: BTA_FM_RDS_MODE_EVT: status=%d, RDS=%s, AF=%s", __func__,
                  (uint8_t)(p_fm_evt_data->rds_mode.status),
        ...
        case BTA_FM_RDS_TYPE_EVT:
            APPL_TRACE_DEBUG("%s: BTA_FM_RDS_TYPE_EVT: status=%d rds_type=0x%x", __func__,
                  (uint8_t)p_fm_evt_data->rds_type.status, (uint8_t)p_fm_evt_data->rds_type.type);
...
{
    bt_status_t status;
    status = btif_transfer_context(btif_fm_upstreams_evt, (uint16_t) event,
        (char*)p_data, sizeof(tBTA_FM), NULL);
...
/** Mute/unmute Fm. */
static int btif_fm_mute(bool mute)
{
    CHECK_BTFM_INIT();
    BTA_FmMute(mute);
    return BTA_SUCCESS;
...
static int btif_fm_read_audio_quality(bool turnOn)
{
    CHECK_BTFM_INIT();
